...
from layers.spectral_norm import spectral_norm
...
class Self_Attn(nn.Module):
    ...
    def __init__(self, in_channels, max_pool_factor=1):
        ...
        # we disable maxpool since ssd has odd hw dim
    ...
    def forward(self, x, return_attn_map=False):
        ...
        assert h == w
        max_pool_size = int(h // self.max_pool_factor)
        if max_pool_size == 0: # edge case for lower-resolution feature map than max_pool_factor
            max_pool_size = 1
        ...
        phi = F.adaptive_avg_pool2d(phi, max_pool_size)
        ...
        # g = self.maxpool(g)
        g = F.adaptive_avg_pool2d(g, max_pool_size)
        ...
        if return_attn_map:
            return out, self.sigma*attn_g, attn
        else:
            return out, self.sigma*attn_g
...
if __name__ == "__main__":
    self_attn = Self_Attn(128)
    x = torch.randn((32, 128, 64, 64))
    y = self_attn(x)